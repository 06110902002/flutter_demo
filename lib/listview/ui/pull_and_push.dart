...
class PullAndPush extends StatefulWidget {
  ...
  PullAndPush({
    ...
    this.defaultRefreshBoxRefreshIconPath = "assets/imgs/refresh.png",
    ...
    required this.headerRefreshBox,
    this.footerRefreshBox,
    required this.animationStateChangedCallback,
    required this.triggerPullController,
...
class PullAndPushState extends State<PullAndPush>
  ...
  late AnimationController animationControllerWait;

  ///记录RefreshBox在动画过程中是触发刷新（加载）的回缩还是刷新（加载）数据完后触发的刷新，
  late RefreshBoxAnimationModel _boxAnimationStatus;
          ...
          switch (_boxAnimationStatus) {
            ...
            case RefreshBoxAnimationModel.PULL_RECOVER_TO_NORMAL:
              ...
              _chenckStateAndCallback(
                AnimationStates.RefreshBoxIdle,
          ...
          new Expanded(
            ...
            child: new NotificationListener(
              ...
              child: ScrollConfiguration(
                behavior: MyBehavior(
                  widget.isShowLeadingGlow,
                  widget.isShowTrailingGlow,
                  widget.glowColor,
                ),
      ...
      setState(() {
        if (notification.dragDetails!.delta.dy / 2 + topItemHeight <= 0.0) {
          ...
          if (topItemHeight > 150.0) {
            ...
            _chenckStateAndCallback(
              AnimationStates.DragAndRefreshEnabled,
...
class MyBehavior extends ScrollBehavior {
  ...
  MyBehavior(
    this.isShowLeadingGlow,
    this.isShowTrailingGlow,
    this._kDefaultGlowColor,
  );

  // 1. 将方法重命名为 buildOverscrollIndicator
  @override
  Widget buildOverscrollIndicator(
    BuildContext context,
    Widget child,
    ScrollableDetails details, // 2. 参数类型变为 ScrollableDetails
  ) {
    // 3. 如果不需要光晕效果，可以直接返回 child
    if (!isShowLeadingGlow && !isShowTrailingGlow) {
      return child;
    }
    // 4. 需要光晕时，返回一个 GlowingOverscrollIndicator
    return GlowingOverscrollIndicator(
      showLeading: isShowLeadingGlow,
      showTrailing: isShowTrailingGlow,
      axisDirection: details.direction,
      // 5. 从 details 获取 axisDirection
...
class TriggerPullController {
  late PullAndPushState pullAndPushState;
